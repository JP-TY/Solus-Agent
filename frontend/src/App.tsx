import { AuthGate, useSolusToken } from "./components/AuthGate";
import { SolusChat } from "./components/SolusChat";
import { SolarSavingsChart, SurveyDatePicker, TariffCard } from "./components/SolusWidgets";

export default function App() {
  const { token, setToken, setEmail } = useSolusToken();
  ...
  return (
    <div className="solus-page">
      ...
      <footer className="solus-foot">
        github.com/JP-TY/Solus-Agent · chat in the sidebar, answers come from the live runtime
      </footer>
      <div className="solus-chat">
        <AuthGate token={token} setToken={setToken} setEmail={setEmail} />
        <SolusChat token={token} />
      </div>
    </div>
  );
}
